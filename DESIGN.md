...
- **Display & Headlines (Newsreader - Serif):** Used for navigation, UI labels, and section titles. It communicates the "Developer" side—precision, logic, and modernism.
  - _Scale Example:_ `display-lg` (3.5rem) for high-impact project titles.
- **Body & Titles (Manrope - Geometric Sans):** Used for case study narratives, "About Me" sections, and storytelling. It communicates the "Designer" side—humanity, history, and craft.
  - _Scale Example:_ `body-lg` (1rem) with a generous `line-height` (1.6) for readability.
